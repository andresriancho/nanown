...
def trainBoxTest(db, unusual_case, greater, num_observations):
    ...
    def trainAux(low, high, num_trials):
        estimator = functools.partial(
            multiBoxTest, {'low': low, 'high': high}, greater)
        estimates = bootstrap3(
            estimator, db, 'train', unusual_case, num_observations, num_trials)
        null_estimates = bootstrap3(
            estimator, db, 'train_null', unusual_case, num_observations, num_trials)
    ...
    for width in widths:
        ...
        # width, statistics.mean(false_negatives),
        # statistics.mean(false_positives)))
        performance.append(
            (abs(statistics.mean(false_positives) - statistics.mean(false_negatives)),
    ...
    num_trials = 500
    widths = [good_width + (x / 100.0)
              for x in range(-120, 125, 5) if good_width + (x / 100.0) > 0.0]
    performance = []
    for width in widths:
    ...
    wt.stop()
    params = json.dumps(
        {"low": best_low, "high": best_low + best_width}, sort_keys=True)
...
def trainSummary(summaryFunc, db, unusual_case, greater, num_observations):
    ...
    def trainAux(distance, threshold, num_trials):
        estimator = functools.partial(
            stest, {'distance': distance, 'threshold': threshold}, greater)
        estimates = bootstrap3(
            estimator, db, 'train', unusual_case, num_observations, num_trials)
        null_estimates = bootstrap3(
            estimator, db, 'train_null', unusual_case, num_observations, num_trials)
    ...
    mean_diffs = [s['unusual_packet'] - s['other_packet']
                  for s in db.subseries('train', unusual_case)]
    ...
    for t in range(80, 122, 2):
        wt.addJob(threshold * (t / 100.0),
                  (good_distance, threshold * (t / 100.0), num_trials))
    wt.wait()
    while not wt.resultq.empty():
        job_id, errors = wt.resultq.get()
        fp, fn = errors
    ...
    for t in range(90, 111):
        wt.addJob(good_threshold * (t / 100.0),
                  (best_distance, good_threshold * (t / 100.0), num_trials))
    wt.wait()
    while not wt.resultq.empty():
        job_id, errors = wt.resultq.get()
        fp, fn = errors
    ...
    wt.stop()
    params = json.dumps(
        {'distance': best_distance, 'threshold': best_threshold}, sort_keys=True)
    return {'trial_type': "train",
            'num_observations': num_observations,
            'num_trials': num_trials,
            'params': params,
...
def trainPyKalman4D(db, unusual_case, greater, num_observations):
    ...
    if _pykalman4d_params == None:
        ...
        _pykalman4d_params = {
            'transition_matrices': kf.transition_matrices.tolist(),
        ...
        _pykalman4d_good_threshold = (
            numpy.mean([m[0] - m[1] for m in smoothed]) + numpy.mean([m[0] - m[1] for m in null_smoothed])) / 2.0
    ...
    params = {'threshold': _pykalman4d_good_threshold,
              'kparams': _pykalman4d_params}

    wt = WorkerThreads(2, trainAux)
    num_trials = 50
    performance = []
    for t in range(-80, 100, 20):
        thresh = _pykalman4d_good_threshold + \
            abs(_pykalman4d_good_threshold) * (t / 100.0)
...
classifiers = {
    'boxtest': {'train': trainBoxTest, 'test': multiBoxTest, 'train_results': []},
