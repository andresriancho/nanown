...
def arctanWeights(derived, trust, alpha):
    ...
    for i in range(len(trust)):
        w = math.pi / 2.0 - \
            math.atan(2 * (trust[i][0] - shift) / (shift - minimum))
        if w < 0.0:
            w = 0.0
        weights[trust[i][1]] = w
...
def arctanWeights2(derived, trust, alpha):
    ...
    for i in range(len(trust)):
        w = math.pi / 2.0 - \
            math.atan(3 * (trust[i][0] - shift) / (shift - minimum))
        if w < 0.0:
            w = 0.0
        weights[trust[i][1]] = w
...
def midsummary(values, distance=25):
    # return (numpy.percentile(values, 50-distance) + numpy.percentile(values,
    # 50+distance))/2.0
...
def ubersummary(values, distance=25):
    ...
    l1, l2, l3, r3, r2, r1 = numpy.percentile(
        values, (left1, left2, left3, right3, right2, right1))
...
def septasummary(values, distance=25):
    ...
    right3 = 50 + (distance / 2.0)
    right1 = (right2 + 100) / 2.0
    l1, l2, l3, m, r3, r2, r1 = numpy.percentile(
        values, (left1, left2, left3, 50, right3, right2, right1))
    return (l1 + l2 + l3 + m + r3 + r2 + r1) / 7.0


def tsvalwmean(subseries):
    weights = [
        (s['unusual_packet'] + s['other_packet']) ** 2 for s in subseries]
...
# def tsvalwmean(subseries):
# return numpy.mean([(s['unusual_tsval']-s['other_tsval']) for s in
# subseries])
...
def bootstrap3(estimator, db, probe_type, unusual_case, subseries_size, num_trials):
    ...
    for t in range(num_trials):
        ret_val.append(
            estimator(db.subseries(probe_type, unusual_case, subseries_size)))
...
def multiBoxTest(params, greater, samples):
    ...
    rest_high, rest_low = numpy.percentile(
