...
class db(threading.local):
    ...
    def populationSize(self, probe_type):
        ...
        try:
            ...
            cursor.execute(
                "SELECT max(c) FROM (SELECT count(sample) c FROM probes WHERE type=? GROUP BY test_case)", (probe_type,))
    ...
    def subseries(self, probe_type, unusual_case, size=None, offset=None):
        ...
        if cache_key not in self._population_cache:
            ...
            self._offset_cache[cache_key] = tuple(
                numpy.random.random_integers(0, len(p) - 1, len(p) / 5))
        ...
        if offset is None or offset >= len(population) or offset < 0:
            ...
            self._cur_offsets[cache_key] = (
                offset + 1) % len(self._offset_cache[cache_key])
    ...
    def _insert(self, table, row):
        ...
        query = "INSERT INTO %s (id,%s) VALUES ('%s',%s)" % (
            table, columns, rid, placeholders)
        # print(row)
    ...
    def addPackets(self, pkts, window_size):
        ...
        self.conn.execute(
            "CREATE INDEX IF NOT EXISTS probes_port ON probes (local_port)")
        cursor = self.conn.cursor()
    ...
    def deleteClassifierResults(self, classifier, trial_type, num_observations=None):
        params = {"classifier": classifier, "trial_type":
    ...
    def setUnusualCase(self, unusual_case, greater):
        ...
        query = """INSERT OR REPLACE INTO meta (%s) VALUES (%s)""" % (
